    ...
    while(numRuns > 0){
        ...
        while(getline(inFile, line)){
            ...
            if(quantAtualVetor < 1000){
                vetorOpera[quantAtualVetor] = stoi(line);
                quantAtualVetor++;
                if(quantAtualVetor == 1000 && particaoAtual == 1000){
                ...
                if(estado == 1 && countLinhas > particaoAtual - 1000){
                    ...
                    count++;
                    if((count >= (1000 - aux) && count < 1000) || (count >= (particaoAtual + aux))){
                        if(count <= 1000){
                            vetorOpera[count - (1000 - aux)] = tempLine;
                        }else{
                            vetorOpera[count - particaoAtual] = tempLine;
                        }

                    }
                    
                    if(inFile.peek() == EOF || count == 1000 + (particaoAtual)){
                        mergeSort(vetorOpera, 0, 1000);
                        for (int i = 0; i < 1000; ++i){
                            outFile << to_string(vetorOpera[i]) << endl;
                        }
